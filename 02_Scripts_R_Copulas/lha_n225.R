...
lha_ <- read.csv("C:/Users/Asus/Desktop/QRM/First Assignment/QRM2020/01_Data/lha_n225.csv", header=T)
lha <- lha_n225 %>% select(2)
n225 <-lha_n225 %>% select(3)

#Check correlation to confirm we get same results as Python and import went correctly
cor(lha,n225,method='spearman')

u <- pobs(as.matrix(cbind(lha,n225)))[,1]
v <- pobs(as.matrix(cbind(lha,n225)))[,2]
...
t.cop <- tCopula(dim=2)
set.seed(500)
m     <- pobs(as.matrix(cbind(lha,n225)))
...
plot(lha$LHA_return,n225$N225_return,main='Returns')
